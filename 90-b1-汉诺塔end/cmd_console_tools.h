...
void cct_cls(void);//清屏
void cct_setcolor(const int bg_color = COLOR_BLACK, const int fg_color = COLOR_WHITE);//恢复缺省颜色
void cct_getcolor(int& bg_color, int& fg_color);
void cct_gotoxy(const int X, const int Y);//光标移动
...
//在xy位置显示x，用x坐标的值做前景色，y坐标的值做背景色（显示两个x是为了使比例协调，可以换成一个x后观察效果）
//清除显示(最后一次保留)，清除方法为用正常颜色+空格重画一遍刚才的位置
void cct_showstr(const int X, const int Y, const char* str, const int bg_color = COLOR_BLACK, const int fg_color = COLOR_WHITE, int rpt = 1, int max_len = -1);/* 在坐标(x,y)位置处连续打印个字符 */
...
void cct_getconsoleborder(int& cols, int& lines, int& buffer_cols, int& buffer_lines);//取现有窗口的大小(窗口的行列、缓冲区的行列)
